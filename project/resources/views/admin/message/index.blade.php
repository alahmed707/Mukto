...
@section('content')  
<input type="hidden" id="headerdata" value="TICKET">
<div class="content-area">
    <div class="mr-breadcrumb">
        <div class="row">
            <div class="col-lg-12">
                <h4 class="heading">{{ __('Support') }}</h4>
                <ul class="links">
                    <li>
                        <a href="{{ route('admin.dashboard') }}">{{ __('Dashboard') }} </a>
                    </li>
                    <li>
                        <a href="{{ route('admin-message-index') }}">{{ __('Support') }}</a>
                    </li>
                </ul>
            </div>
        </div>
...
{{-- DELETE MODAL ENDS --}} {{-- MESSAGE MODAL --}}
...
@section('scripts')
    ...
    var table = $('#geniustable').DataTable({
        ...
        $(".btn-area").append('<div class="col-sm-4 text-right">'+
          '<a class="add-btn" href="javascript:;" data-toggle="modal" data-target="#vendorform"><i class="fas fa-envelope"></i> {{ __('Send Message') }}</a>'+'</div>');
      });                       
